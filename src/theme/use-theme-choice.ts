...
export function useThemeChoice() {
  ...
  // Resting on a theme in the menu tries it on: the whole app, canvas included, repaints.
  // Only a click makes it stick — leaving the menu puts back the one that was chosen.
  ...
  // The pointer must dwell: a theme swept over on the way to another is not a theme asked for.
  ...
  // The canvas palette lives in a module binding rather than in React state, so it is repointed before the first paint of the new theme, not after it.
  // It then fades towards it, in step with the interface — except on the very first paint, which has no previous theme to fade from.
  ...
  // The menu stays open on a choice, as it does for the grid switches above it: ambience and family are two controls, and one is rarely set without a look at the other.
