/**
 * The ruler loads are drawn on: how a magnitude becomes a length in screen px, and back.
 * Pure arithmetic — nothing here knows where a load sits.
 *
 * The ruler is logarithmic so that loads of very different magnitudes stay legible side by side.
...
// ─── Display scale ──────────────────────────────────────────────────────────
// One decade of magnitude, one fixed step of drawn length, from `FLOOR_VALUE` up: every decade of the range is worth the same on screen, so a load of a few newtons is as distinguishable from its neighbours as one of a few kilonewtons, and the whole plausible range still fits on screen.
// Values below `FLOOR_VALUE` all draw at `MIN_PX` — the ruler has no bottom of its own.
...
/**
 * Drawn radius (screen px) of a moment's arc, on its own ruler (`MOMENT_SCALING`) rather than `stored2screen_load`'s — moments live in a different typical range than forces.
 * Unsigned: the sign of a moment is its rotation direction, which `draw_moment` reads separately — a negative radius would just throw out of `ctx.arc`.
 */
...
/**
 * The round value nearest to `value`, on the ladder a drag snaps to: the mantissas of `ruler.SNAP_MANTISSAS` in every decade, floored at `ruler.MIN_VALUE`.
...
export function nearest_round_load_value(
  ...
  // The neighbouring decades matter: just under 1000, the nearest rung up is the next decade's 1, not this one's 5.
...
// The log ruler exists to keep loads of very different magnitudes legible side by side — a comparison *between* loads.
// Inside one load the reading is a different thing entirely: an engineer reads the intensity at any point off the straight line joining the two tip arrows, which only means anything if the drawing is proportional to the values across the span.
// So the compression is applied once, to the load's dominant magnitude, and the whole profile is drawn linearly from there: the biggest arrow keeps the length the ruler gives it, the arrow at mid-span is exactly the average of the two, and a triangular load draws as a true triangle instead of bottoming out on `MIN_PX`.

/**
 * Below this, a magnitude counts as zero.
 * It is the threshold the canvas already applies without saying so: `draw_dimension_text` keeps one decimal, so anything under it is written "0" anyway.
 */
...
/**
 * Signed drawn length of one endpoint arrow, `other` holding the magnitude at the opposite end — it shares the gain, so it takes part in the result.
 */
...
/**
 * Inverse of `distributed_tip_length`: the magnitude a tip dragged to the signed drawn length `length` takes, the opposite end holding `other`.
 *
 * The gain follows the dominant magnitude, so the drag has two regimes.
 * While the dragged tip is the dominant one, its length reads straight off the log ruler — and the opposite arrow rescales as the gain moves under it.
 * Once it falls below the other end, the gain is pinned by that other end and the tip moves linearly, which is what lets it reach exactly zero and turn the load triangular.
 * The two regimes agree where they meet.
 */
...
/**
 * Signed drawn length of the profile at the point of the beam where the body bar was grabbed, when the magnitude there is `magnitude`.
 * `offsetStart` and `offsetEnd` are that point's differences to the two endpoint magnitudes — constants for the whole drag, since a body drag translates both ends by the same amount.
 */
...
/**
 * Inverse of `distributed_grab_length`, by bisection: there is no closed form, because the gain moves with the load's peak and so sits on both sides of the equation.
 * The length is continuous and increasing in the magnitude (zero at zero, growing like the log ruler at either end), which is all a bisection needs; the bracket is widened by doubling first since the drawn length grows only logarithmically.
 */
