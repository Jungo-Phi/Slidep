 ...
 * One pulley makes a band around a single wheel: geometrically drawable, but it transmits nothing and its two terminals are the same point.
 */
...
/**
 * How many pulleys a belt being placed runs over: its route, plus the gear the gesture started on when the route does not list it yet — a start on a gear only enters the route once a next via exists to orient its wrap.
 */
...
/**
 * Whether the belt's loop physically exists: enough pulleys, and both terminals held by one junction node.
 *
 * `closed` must equal this — the equivalence holds both ways.
 * Losing a pulley opens the belt; joining both terminals onto one node closes it.
 * A belt whose flag disagrees is incoherent, not merely untidy.
 */
...
/**
 * The pulley a belt terminal wraps: the first attached gear for the start terminal, the last for the end.
 * A terminal may rest ON that pulley's rim — it is then a wound end — but never inside it.
 */
