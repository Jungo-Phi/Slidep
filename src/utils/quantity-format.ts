/**
 * Formatting and parsing for numbers that carry a physical unit, shared by every input that edits one — the properties panel's `NumberInput` and the canvas's `OnCanvasValueEditor`.
 *
 * A value is always stored and passed around in SI (metres, radians, newtons, kilograms…).
 * Display and parsing convert at the edge, the same way `viewport.ts` is the only place screen and world coordinates cross.
 */
...
export interface QuantityKind {
  /**
   * Fixed: always shown and, absent an explicit unit in the input, always read in `units[0]` — length and angle, whose values live in one narrow, human-familiar range regardless of mechanism size.
   *
   * Adaptive: display picks whichever SI prefix on `baseUnit` keeps the mantissa in [1, 1000) — force, stiffness, mass… whose values can land anywhere across many decades depending on the mechanism, so no single fixed unit stays readable throughout.
   */
  adaptive: boolean;
  /** Tried in listed order for a typed unit — longest/most specific symbols first, so "mm"
   * matches before a bare "m" would.
   * `units[0]` is what a bare number (no unit typed) means, and what a fixed kind always displays in.
   * Ignored (beyond `[0]`) by an adaptive kind, which generates its own ladder from `units[0].symbol` as the SI base unit. */
  units: QuantityUnit[];
  /** Where an adaptive kind's prefix ladder earns a name of its own rather than going on
   * stacking SI prefixes on the same base symbol — mass does, at the tonne: 1e6 g (the "Mg" slot) reads "T", 1e9 g ("Gg") reads "kT".
   * From `exp` up, `display_unit` restarts the ladder on `symbol` instead of `units[0].symbol`. */
...
export const ANGLE: QuantityKind = {
  adaptive: false,
  // "deg" first: what a field displays and, typed bare, what it reads — easier to type and read than "°" (no dead key, no ambiguity with a stray degree sign), which still parses.
  units: [
...
/**
 * `angleRad` folded into [0, 2π) — a compass heading, not a signed turn.
 * `Point2.angle()` answers `atan2`'s own [-π, π], which a direction field displays as its positive equivalent instead: showing 315° rather than -45° for a field meant to read as an orientation.
 */
 ...
 * Past 1e6 g the ladder earns the tonne's own name instead of stacking further prefixes on "g" — see `renamedFrom`. */
...
/**
 * A motor's speed — the one quantity whose unit symbol is localised (`unit_rpm`): "tr/min", "rpm", "U/min" depending on language, unlike every SI symbol elsewhere, which is the same word in all four.
 * A function, not a constant, so it re-reads the current language on every call rather than freezing whichever was active at module load.
 * Every language's spelling still parses regardless of which is active, the same way `ANGLE` accepts "rad" typed over its own default "deg".
 */
...
export const ANGULAR_VELOCITY = (): QuantityKind => ({
  ...
  units: [
    ...
    // Revolutions per minute: French, English, German spellings, plus the physicist's "min⁻¹" (a revolution is implicit — the unit only names the time it's counted over), in both its typeable ("min-1") and true-superscript form.
...
export function display_unit(
  ...
  // In units of `base`, not of `valueSI` — the two coincide everywhere but `MASS`, whose base (the gram) isn't the unit `valueSI` is actually stored in (the kilogram).
  const magnitude = Math.abs(valueSI) / base.factor;
  // A diverging solver can feed a NaN/Infinity value through here — fall back to the base unit rather than let an unbucketable exponent reach the `SI_PREFIXES.find` below.
  if (magnitude === 0 || !Number.isFinite(magnitude)) {
    return { symbol: base.symbol, factor: base.factor };
  }
  // Rounded before the bucket is picked, not for the mantissa itself: `floor` is discontinuous exactly at a bucket edge, so a magnitude landing a single ULP under a round one (1 N·m read back as 0.9999999999999999, say — routine for a value a live solver recomputes every frame) would otherwise flip the whole unit rather than just its last displayed digit.
  // Nine significant digits is far below any precision a caller ever displays and far above the noise floor of an iterative double-precision solver, so a genuinely different value is never affected — only a bucket edge is.
...
/**
 * `valueSI` formatted for display: the mantissa in whichever unit `kind` shows it in, the unit's symbol after a space.
 */
...
/**
 * Whether two SI values are indistinguishable in a field showing `kind` at `precision` — the only sense in which several elements can be said to agree on a value.
 *
 * Comparing the doubles instead answers a question nobody asked: a solver leaves two lengths it was told to make equal differing in their last bits, and a field would then have to claim they are mixed while showing the same number for each of them.
 */
...
/**
 * `valueSI`'s mantissa alone, no unit symbol — a permanent on-canvas length dimension, say, where the unit is fixed and known (mm) and showing it on every single label would only be noise.
 * Reserved for a `kind` whose reader already knows the unit without being told; an adaptive kind's unit is informative (it says which decade the value is in) and should keep using `format_quantity` instead.
 */
 ...
 * Applied to both sides of a comparison, never to what gets displayed.
 */
