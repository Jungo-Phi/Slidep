/**
 * A gear's mass properties. It is modelled as a solid disk of uniform surface mass: `m = mₛπr²`
 * and `J = ½mr²`. Both the dynamic mass model and the properties panel's editable inertia read
 * the disk from here, so the field and the solver can never disagree.
 */
 ...
 *  `gear_inertia`, which the panel's J field writes mₛ through. 0 for a degenerate radius,
 *  rather than the infinity the division would give. */
