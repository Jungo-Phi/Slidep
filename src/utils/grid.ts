 ...
 *
 * Two different ladders read this same geometry: `grid_snap_step` (where a placed point
 * lands) follows the grid's own drawn hierarchy — `draw_grid`'s opacity levels emphasise
 * multiples of 1, 5 and 10 within a decade, so snapping to exactly those is snapping to a line
 * that is actually there to be seen. `graduation_step` (which lines get a number) instead
 * follows the roundest, most legible progression — the two agree on the coarsest and finest
 * rung but not on what falls between, and neither has to: a labelled number is information to
 * read, a snapped point is a place to land, and they don't need the same ladder to serve their
 * own purpose well.
 */

/**
 * Where in `[0, 1)` each rung of `ladder` takes over — derived from the ladder itself rather
 * than tuned by hand, so the sequence of round numbers stays the only thing anyone has to
 * choose.
 *
 * Rung `i`'s width is set to `log10(ladder[i] / ladder[i+1])`: exactly the drop screen space
 * takes when the *next* rung's switch arrives. That makes every rung's low point land back at
 * the same reference level (its own switch always costs it precisely what it was given), so
 * the whole pitch envelope ends up exactly as tight as its single widest gap — the best any
 * threshold placement could do for this ladder, with no search required (see
 * `docs/grille-adaptative.md` §5 for the derivation).
 *
 * The one seam this can't cover: the finest rung's switch, at the decade's end, carries no gap
 * of its own — it hands off to the next decade's coarsest rung at the same value, not a
 * smaller one (`grid_metrics`'s own continuity, below). That first gap, between the coarsest
 * rung and the one after it, is therefore split evenly between the coarsest and finest rungs
 * instead of charged to the coarsest alone.
 */
 ...
 *  drawn hierarchy (`draw_grid`'s opacity levels single out multiples of 1, 5 and 10; there is
 *  no separately-emphasised "2"), so a snapped point always lands on a line that is actually
 ...
 *  decision `CALIBRATION` exists to satisfy. Below this a hold of a few px would cover a
 *  meaningful fraction of the visible plane, and one could barely put a point down off a
 *  line. */
export const LABEL_PITCH_FLOOR_PX = 40;

/** Its mirror at the other end, a direct consequence of `LABEL_PITCH_FLOOR_PX` and
 *  `GRADUATION_LADDER`'s own widest gap rather than a second choice to make — see
 *  `rung_thresholds`'s doc for why that gap is unavoidable. Exported so a test can check the
 *  envelope without re-deriving or hard-coding either end of it. */
...
/**
 * Exponent offset that lands `GRADUATION_LADDER`'s own floor exactly on
 * `LABEL_PITCH_FLOOR_PX` — derived from the ladder rather than tuned by hand, so a future
 * change to it keeps the same floor instead of silently drifting off it.
 *
 * This is the one dial for the whole adaptive grid: raising `LABEL_PITCH_FLOOR_PX` pushes
 * every labelled line further apart, and — since the unlabelled, finest lines and `SNAP_LADDER`
 * ride the same `local`/`decade` as the graduations (drawing and snapping cannot disagree on
 * where a line is) — pushes them apart too, proportionally. The finest lines are no longer
 * independently centred on a fixed ~10 px the way a hand-picked constant once did; their range
 * now scales with `LABEL_PITCH_FLOOR_PX` instead.
 */
 ...
 *
 * Always a multiple of the drawn grid's own step, and of `SNAP_LADDER`'s own rung — so a
 * snapped point lands exactly on a line the grid draws with emphasis, not an arbitrary one.
 */
...
/**
 * World distance between two labelled axis graduations — see `draw_graduations` in
 ...
 *  both to detect the decade-start repeat (comparing against `graduation_multiple(0)`) and to
 *  size the decimals a label needs. */
