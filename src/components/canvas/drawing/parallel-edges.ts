/**
 * Where a spring and a damper sharing two nodes are drawn, so they read as two elements in parallel rather than one on top of the other.
 *
 * Only the drawn body moves: an edge's `positionStart` and `positionEnd` stay on their nodes, so everything anchored to it — loads, dimensions, the probe badge, the solver — keeps reading the same geometry.
 * What the offset must not do is disagree between the stroke and the cursor, which is why drawing and hit-testing both come here.
 */
...
/**
 * Which side each type takes.
 * Keyed on the type rather than on an ordering of ids, so the pair never swaps sides between two frames — and the coexisting pair is exactly these two types.
 */
 ...
 *
 * Built for the whole mechanism in one pass: it is read once per frame by the drawing and once per move by the hover, both of which would otherwise pay a scan per edge.
 */
