...
/**
 * The curves the "Bilan énergétique" chart can show — see `AnalysisPanel.tsx`, toggled the
 * same way a vector probe's x/y/norm are. `kinetic`, `potential` and `mechanical` are the raw
 * values `EnergySample` computed that frame — `potential`'s own zero is an arbitrary
 * coordinate-origin artifact of wherever the mechanism sits in the drawing, not a physical
 * one, but shown absolute anyway (a deliberate choice: relative-to-start hid the real number
 * at frame zero, which is exactly what this chart is for). `netWorkIn` is a genuinely
 * different kind of quantity — a running integral of work, not a state — so it keeps its own
 * natural zero at the start of the recording regardless.
 */
export interface EnergyBalanceSeries {
   ...
   *  undamped mechanism (a pendulum, a bouncing mass) while their sum, `mechanical`, stays
   *  flat — its own zero is wherever the drawing's coordinate origin happens to be, so only
   *  ITS CHANGE across the recording is physically meaningful, not the number itself. */
  potential: number[];
  /** J — `kinetic + potential`. Flat for a mechanism the solver treats as exactly
   *  conservative; any real drift (see docs/plan-analyse-ddl.md's ~0.98 spectral radius) shows
   *  up as a slope with nothing on `netWorkIn`'s own CHANGE to match it. */
  mechanical: number[];
  /**
   ...
   * the two is the whole point of this chart.
   */
  netWorkIn: number[];
}

/** Empty when nothing has been recorded, or nothing in it yet carries an `energy` sample —
 *  the same "not yet analysable" meaning an empty result carries everywhere else in this
 *  panel, never a mechanism that has none of these quantities. */
