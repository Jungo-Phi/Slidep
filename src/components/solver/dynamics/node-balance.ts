 ...
 *
 * This is the check `CohesionField.loopResidual` structurally cannot make.
 * That one compares a beam's own march against its own far-end reading — both derived from the same torsor, so a boundary torsor that is wrong in the same way at both ends closes it perfectly (measured: a beam holding 5 t reads 7.7 N with a residual of 0.002 N).
 * A node balance confronts a beam with everything ELSE coincident there — its neighbours, the loads, the mass lumped on it — and so can only close when the readings agree with the mechanism.
 *
...
export interface NodeBalance {
  ...
  atAnchor: boolean;
  /**
   * `Σ F − m·a` at this node.
   * Zero on a frame whose readings are consistent with the mechanism; anything else is a defect in the readings, not a property of the mechanism.
   ...
   * inertia, so this is zero on a consistent frame.
   * Always zero at an anchored node, where `reactionMoment` takes it instead. */
  momentResidual: number;
  /** What the frame pushes back with, at an anchored node only. */
  reaction?: Point2;
  /**
   * The couple the frame supplies, at an anchored node only.
   * Whether the support can really supply one is a separate question — a welded support can, a pin cannot — and this says nothing about it: it reports what the beams leave over, which is the support moment exactly when there is a weld to carry it.
   */
  ...
  scale: number;
  /**
   * False when this node carries a link the balance does not model — a belt strand, a gear mesh, a contact.
   * Its residual then measures what the model leaves out rather than what the readings get wrong, and says nothing about the beams.
   */
  covered: boolean;
}

/**
 * Link types whose action at a node is already accounted for by the beam torsors and the external forces this balance sums.
 * Everything else — belts, gear meshes, contacts, a beam welded to a gear hub — transmits force the balance never sees, and marks its nodes `covered: false` rather than reporting a residual it cannot attribute.
 */
const MODELLED_LINK_TYPES = new Set([
  ...
  "Angle",
  "KeepOrientation",
  // Dropped from the sweep in dynamic mode and applied as real forces instead (`step_dynamic_simulation`), so `externalForceAt` already carries them.
  "Spring",
  "MotorBeam",
  "MotorAngle",
]);
 ...
 *
 * `mass-model.ts` lumps `BEAM_END_MASS_FRACTION` of a beam onto each of its endpoint nodes, so a node's solver mass holds material that is really the beams'.
 * The cohesion torsor is a cut through a CONTINUUM whose whole linear mass `cohesion-field.ts` marches against, so that material sits inside the beam, on the far side of the cut at `0⁺` — counting it again as the node's own would ask the beams to hold their own endpoints up.
 */
 ...
 *
 * Pure, and deliberately fed by accessors rather than a snapshot: the keys here are FUSED ones, which a snapshot layout does not index, and the caller is the only one that knows how its own state resolves them (`step_dynamic_simulation` has the maps, a panel has to unfuse).
 *
 * `externalForceAt` is everything applied at a node that is not a beam and not its own weight: loads, spring/damper forces, a motor's force couple — the same map the dynamics step folds into its predict acceleration.
 * Gravity is added here from `inverseMassOf`, so it must NOT be included (`step_dynamic_simulation`'s own `groundedWeights` restatement is a solver artefact and belongs out of it too — an anchored node's weight comes from `groundedMasses`).
 */
export function resolve_node_balance(
  ...
  // A beam's own midpoint is internal to it — `resolve_beam_cohesion` folds that link's reaction straight into `start`/`end`, so no beam reports an action there and its balance could only ever read as a phantom gap.
