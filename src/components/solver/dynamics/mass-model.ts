...
/**
 * Fallback mass for a node with NOTHING physical lumped onto it — an isolated point, or one connected only to springs/dampers, neither of which carries mass of its own — so it never divides by zero.
 * Reuses the point-mass default rather than inventing a separate constant: a node with nothing to lump onto it behaves like an implicit 1 kg point mass.
 *
 * Only ever substituted for an EXACT zero (`floored_mass` below) — a small but real lumped mass (a light beam's own midpoint, `mass-model.ts`'s `BEAM_END_MASS_FRACTION` share) must stay exactly what it is, however far under 1 kg: flooring it too inflates that beam's own self-weight/inertia by however much 1 kg exceeds its true mass, silently, in both the dynamics itself and anything reading its reactions (`beam-cohesion.ts`).
 */
