...
/**
 * Precomputed once at compile time (`compile_simulation_model`): for one beam, its fused endpoint keys, the nodes attached to its span, and its own mass.
 *
 * This is the shape of a beam as a BODY, which is what the statics pass needs of it — see docs/plan-efforts-interieurs.md phase 10.
 * A beam's endpoints are fused with whatever else is coincident there, so nothing downstream can recover them from the raw mechanism; they are resolved here, once, from the beam's own rigid-length `Distance` link.
 */
export interface BeamCohesionSpec {
   ...
   *
   * Read only to know each endpoint node's share of it (`BEAM_END_MASS_FRACTION`), which belongs to the beam and not to the node — see `statics-frame.ts`'s `nodeMassAt`.
   */
  mass: number;
}
...
export function build_beam_cohesion_specs(
  ...
  for (const beam of mechanism.mechanicalElements) {
    ...
    // The beam's own rigid-length link is what fixes k0/k1 — always exactly one, built unconditionally for every beam in `get_links_simulation`.
