 ...
 * Returns the largest gap a drive still below its limit started the pass with, in radians — what the sweep's convergence has to wait for, since a saturated drive has nothing left to close.
 */
export function apply_drives(nodes: SimNodes, drives: ResolvedDrive[]): number {
  ...
  for (const d of drives) {
    ...
    if (!saturated) worst = Math.max(worst, Math.abs(c));
