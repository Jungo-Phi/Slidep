...
import { build_statics_system } from "../statics/equilibrium-model";
import { StaticsSolution, solve_statics } from "../statics/equilibrium-solve";
import { statics_frame } from "../statics/statics-frame";

/**
 * The statics pass against answers computed by hand — see docs/plan-efforts-interieurs.md
 * phase 10. Nothing here reads a solver reaction: the point of these two cases is that the
 * readings which DO (`BeamCohesion`) get them wrong, each in its own way, and that neither
 * failure survives solving equilibrium instead.
 */
...
function solved(json: string, frames: number) {
  ...
  const system = build_statics_system(
    ...
    (key) => (model.dynamicMasses.posMasses.get(key) ?? 1) <= 0,
  );
...
describe("statique d'un cantilever soudé en deux tronçons", () => {
  // 10 N at the tip over 15.402 N of self-weight per half. Statics answers the root exactly:
  // 40.804 N and 25.402 N·m, and the mid-span moment is 8.850 N·m.
  ...
  it("la soudure entre deux poutres mobiles est action-réaction", () => {
    // What the reaction reading cannot do: the `Angle` link of a `join` names both beams'
    // endpoints, so both claim it and both count its whole contribution. Solving equilibrium
    // never asks who owns it — the joint's own balance settles it.
    const { solution } = once(cantileverJson, 400);
    ...
    // And the couple it passes is the beam's own bending moment there, not zero — which is
    // what the reaction reading loses entirely.
...
describe("statique d'un treillis chargé", () => {
  ...
  it("les réactions d'appui bouclent sur la statique", () => {
    ...
    const vertical = support(solution)
      .map((t) => t.fy)
      .sort((a, b) => a - b);
    // A support torsor is what the FRAME applies onto its node, so it reads as the classical
    // support reaction directly: `Dicu` pushes down, `Uqin` up.
  ...
  it("rend un résidu, et il est petit devant les efforts", () => {
    // Not zero: the suspended mass is still swinging, so the frame is not in equilibrium and
    // the d'Alembert terms do not close exactly. Reported rather than hidden.
    const { solution } = once(trussJson, 600);
