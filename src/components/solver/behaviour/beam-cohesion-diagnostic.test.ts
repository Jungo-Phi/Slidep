...
describe("diagnostic docs/plan-efforts-interieurs.md point 2 — deux poutres réunies par un pivot", () => {
  it("force_at somme les deux poutres à la clé fusionnée, au lieu d'isoler l'effort propre à A", () => {
    ...
    // What we get: the fused-key sum F_A + F_B, not F_A alone.
    expect(atBeamAEnd!.vector.x).toBeCloseTo(0, 0);
    expect(atBeamAEnd!.vector.y).toBeCloseTo(100, 0);

    // What beam A alone actually carries — the true cohesion reading phase 3 has to produce.
    // The current value is off by exactly F_B, not a rounding error.
    expect(atBeamAEnd!.vector.x).not.toBeCloseTo(50, 0);
    expect(atBeamAEnd!.vector.y).not.toBeCloseTo(50, 0);
  });
});
