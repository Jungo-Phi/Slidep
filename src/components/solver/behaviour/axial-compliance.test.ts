...
/**
 * On a hyperstatic structure the share of load between members is a matter of their
 * stiffnesses — statics alone does not determine it. A rigid solver has no stiffness to
 * answer with, so it answered with its sweep order instead; `Distance` links now carry the
 * beam's own axial compliance `L/EA` (`beam_axial_compliance`), which is what this guards.
 *
 * Softening ONE member and watching the others' share move is the whole test: the absolute
 * figures are the solver's business, the dependence on EA is the physics.
 */
...
describe("raideur axiale d'un treillis hyperstatique", () => {
  it("chaque poutre porte une compliance, et la répartition la suit", () => {
    ...
    // A member ten times softer sheds load onto its neighbours. Measured as a share of each
    // reading rather than in newtons: what must hold is that SOME member's share moves by
    // more than solver noise, which a rigid solver could not produce at all.
    ...
    // …and the members carrying the statically determinate path barely move: their share is
    // not the redundancy's to redistribute.
