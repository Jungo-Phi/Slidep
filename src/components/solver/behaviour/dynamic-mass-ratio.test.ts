...
// CP.slidep: a grounded joint carrying a light beam, with a pivot rigidly riding that beam's own midpoint (FixedOnSegment) and free to rotate, and a second beam from that pivot to a hanging mass.
// A rigid constraint's position accuracy should not depend on what it is asked to hold — only the reaction (force) does — so the pivot straying off the beam's midpoint is purely a measure of how well the solver converged, not of anything physical.
//
// What it takes to hold: a dynamics step exits on the same residual as a kinematic one, and the sweep order alternates on an acyclic chain, which turns the round trip Gauss-Seidel needs into a direct solve — see docs/ratio-masse-convergence-dynamique.md.
// Without either, the residual grows with the mass ratio instead of staying bounded.
