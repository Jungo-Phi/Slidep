...
/**
 * A motor anchored to a beam (rather than the ground) drives its children relative to that beam's own orientation.
 * `compile_simulation_model` fuses coincident position keys and rewrites every link's own key fields to the fused ones — a field the rewriter does not know about keeps pointing at a key that fusion just deleted, so any position it reads back silently reads as absent.
 * That is what left an anchored motor frozen at its starting angle regardless of speed: it read as though the driven beam had a `KeepOrientation` constraint.
 */
...
describe("moteur ancré sur une poutre", () => {
  it("fait tourner son enfant, pas seulement le moteur au sol", () => {
    ...
    // A fixed pedestal (ANCHOR, both ends grounded) the motor pushes against, and a free arm (DRIVEN) with nothing else constraining its angle around HUB.
    ...
    // Well past a quarter turn at 1 rev/s, and past the motor's own startup ramp (see `MOTOR_STARTUP_RAMP_S`) — 90 frames at 120 fps is 0.75 s, half of it spent ramping up to speed, still leaving a half turn done at full speed once it has.
    for (let i = 1; i <= 90; i++)
      snapshot = step_simulation(model, i * RECORD_DT, snapshot);
    const quarterTurn = snapshot_point(snapshot, `${DRIVEN}:end`)!;

    // Frozen (the bug): stays within solver noise of where it started.
    // Rotating (fixed): a 100 px arm swung well past a quarter turn moves its tip by well over 100 px.
