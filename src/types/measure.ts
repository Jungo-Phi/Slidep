 ...
 *
 * Everything here names elements rather than points, so a reading follows the mechanism as it
 * moves — a ruler laid between two pivots goes on measuring them while a simulation runs. An
 * end laid on empty space keeps a point of its own, and stays where it was put.
 */
...
export type MeasureAnchor =
   ...
   *  end can say — the nearest one. Its centre is reached through its axle, which is a node
   *  like any other, so nothing here has to choose between the two. */
  | { kind: "gear"; gearID: ID }
 ...
 *
 * A distance spans two ends. An angle is not a span at all — it has a vertex, one of four
 * quadrants, and an arc — so it holds the bars themselves rather than two points. A radius
 * belongs to one gear alone. Which of the three a gesture produces is settled by its second
 * click, as the dimensioning tool settles its own kind.
 */
export type Measure =
  ...
  | {
       ...
       *  Settled once, at the second click — recomputing it as the bars swing would let the
       *  reading jump to the opposite angle mid-simulation. */
      flipStart: boolean;
      flipEnd: boolean;
