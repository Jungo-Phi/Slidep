...
Suite de [belt-closed-diagnostic.md](./belt-closed-diagnostic.md), qui avait établi que le
...
Harnais : [belt-transmission-diagnostic.bench.test.ts](../../src/components/solver/belt-transmission-diagnostic.bench.test.ts).
Le second harnais de ce tour, `corexy-slip-diagnostic.test.ts`, a été supprimé : il n'assertait
rien et mesurait `Core XY modifié`, qui n'est plus un banc de référence (README §7). Ses résultats
sont ceux de cette note.
