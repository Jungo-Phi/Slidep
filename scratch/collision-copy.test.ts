...
import type { BeamElement, ID, MassElement, MechanicalElement, PivotElement } from "../src/types/element";
...
describe("instrument", () => {
  it("sweeps", () => {
    ...
    const M = id(), A = id(), B = id(), F = id();
    const m = mechanism([mass(M, new Point2(50, 100)), pivot(A, new Point2(0, 0)), pivot(B, new Point2(100, 0)), beam(F, new Point2(0, 0), new Point2(100, 0), A, B)]);
    const model = compile_simulation_model(m);
    let s: DynamicSnapshot | null = null;
    for (let i = 0; i < 700; i++) s = step_dynamic_simulation(model, i * RECORD_DT, s, RECORD_DT, GRAVITY, undefined, undefined, undefined, true);
    writeFileSync("C:/Users/arnol/Documents/slidep/scratch/collision-sweeps.txt", `extent=${model.extent}\n` + log.join("\n"));
