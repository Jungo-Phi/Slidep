...
Lorsqu'on déplace un élément dans le canvas, une "pseudo-simulation" est effectuée. Ce n'est pas une simulation physique complète (moteur de physique), mais un ensemble de règles de propagation de mouvement basées sur les contraintes de dépdéfinies dans [`mechanical-elements-description.md`](_bmad-output/planning-artifacts/mechanical-elements-description.md).
...
6.  **Contraintes Géométriques Avancées (CAO)** : Le système doit supporter des relations logiques entre éléments (parallélisme, perpendicularité, égalité de longueur, alignement).
